...
export default function LobbyManagement() {
  ...
  // 🔹 Online Users Logic (unused in UI currently but good for future)
  ...
  useEffect(() => {
    ...
    socketRef.current = initLobbySocket({
      onUpdate: (data) => {
        // Firestore listeners handle main sync
      },
  ...
  // 🔧 Helper to get member list (Expects Phone Numbers)
  const getTeamMemberIds = (team) => {
    if (Array.isArray(team.members)) return team.members;
    if (Array.isArray(team.participants)) return team.participants.map(p => p.phone || p);
    return [];
  };

  // 🏷️ Find Participant Info (By Phone)
  const getParticipantInfo = (phone) => {
    const p = participants.find(p => p.phone === phone);
    return p ? { name: p.name, phone: p.phone } : { name: phone, phone: phone };
  };

  // 🔍 Calculate Unassigned (Currently online & waiting in lobby, not in a team)
  const unassigned = participants.filter(
    (p) =>
      onlineUsers.includes(p.phone) &&
      !teams.some((t) =>
        getTeamMemberIds(t).includes(p.phone)
      )
  );
  ...
  // ➖ Remove Member Logic
  const handleRemoveMember = (teamId, phone) => {
    // Visual confirmation
    if (!window.confirm(`Remove ${phone} from this team?`)) return;
      ...
      <div className="mx-auto max-w-6xl rounded-2xl bg-slate-900/80 border border-slate-800 p-5 sm:p-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[2fr,1fr] gap-6">
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {memberIds.map((enrollment, index) => {
                      const { name, phone } = getParticipantInfo(enrollment);
                      return (
                        <div
                          key={index}
          ...
          <div className="space-y-2 max-h-[1000px] overflow-y-auto">
            ...
            {unassigned.map((p) => {
              const isOnline = onlineUsers.includes(p.phone);
              return (
                <div
                  key={p.id}
                ...
                >
                  <div className="flex items-center gap-2">
                    <div
                      className={`w-2 h-2 rounded-full ${isOnline ? 'bg-green-500' : 'bg-slate-600'}`}
