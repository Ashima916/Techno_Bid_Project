...
const AddParticipant = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const duplicate = participants.find(p => p.phone === formData.phone && p.id !== editingId);
    if (duplicate) {
      setMessage({ text: 'This phone number is already registered!', type: 'error' });
      return;
    }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4">
          <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6 shadow-xl backdrop-blur-sm sticky top-6">
            ...
            {message.text && (
              <div
                className={`p-4 mb-6 rounded-xl text-sm font-medium border ${message.type === 'error'
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="pt-4 space-y-3">
                <button
                  type="submit"
                  disabled={loading}
                  className={`w-full py-3 px-4 rounded-lg font-semibold text-sm shadow-lg transition-all duration-200 ${loading
